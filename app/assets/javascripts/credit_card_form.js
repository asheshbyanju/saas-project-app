...
function GetURLParameter(sParam) {
  ...
  var sURLVaraibles = sPageURL.split('&');
  for(var i = 0; i < sURLVaraibles.length; i++)
  {
    var sParameterName = sURLVaraibles[i].split('=');
...
// function to handle the submit of the form and intercept the default event
  submitHandler = function(event) {
    ...
    if(Stripe){
      ...
      show_error("Failed to load credit card progessing functionality. Please reload the page")
    }
    return false;
  };  
...
//Set up plan change event listener #tenant_plan id in the form of class cc_form
  $("#tenant_plan").on('change', function(event) {
    handlePlanChange($('#tenant_plan :selected').val(), ".cc_form");
  });  

// cal plan change handler so that the plan is set correctly in the drop down when the page load
